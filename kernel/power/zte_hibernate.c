...
struct hibernate_alarm {
	struct rtc_timer		rtctimer;
	struct rtc_device *rtc;
} hb_rtc_alarm;

void hibernate_timer_update(void *private)
{
...
void setup_hibernate_alarm(struct rtc_device *rtc, unsigned long timeout)
{
	...
	ktime_t now;
	if (hb_rtc_alarm.rtc) {
		rtc_timer_cancel(hb_rtc_alarm.rtc, &hb_rtc_alarm.rtctimer);
	}
	hb_rtc_alarm.rtc = NULL;
	if (timeout > 0) {
		int err = 0;
		rtc_read_time(rtc, &tm);
		now = rtc_tm_to_ktime(tm);
		now = ktime_add_us(now, (u64)timeout*USEC_PER_SEC);
		err = rtc_timer_start(rtc, &hb_rtc_alarm.rtctimer, now, ktime_set(0, 0));
		if (!err) {
			hb_rtc_alarm.rtc = rtc;
		} else {
			printk(KERN_ERR"(%s:%d) fail to start hibernate timer(%lus)\n", __FUNCTION__, __LINE__, timeout);
		}
	}
	return ;
}
EXPORT_SYMBOL_GPL(setup_hibernate_alarm);
...
init_hibernate_proc(void)
{
	...
	hb_rtc_alarm.rtc = NULL;
	rtc_timer_init(&hb_rtc_alarm.rtctimer, hibernate_timer_update, NULL);
