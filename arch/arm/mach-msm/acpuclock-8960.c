...
#if 1
#define BOOST_UV_FAST 25000
#define BOOST_UV_NOM  25000
#define BOOST_UV_SLOW 25000
#else
#define BOOST_UV_FAST 150000
#define BOOST_UV_NOM  100000
#define BOOST_UV_SLOW 50000
#endif
...
extern const char* read_zte_hw_ver(void);
static struct acpu_level * __init select_freq_plan(void)
{
	...
	if (cpu_is_msm8960()) {
		...
		case 0x7:
			...
			enable_boost = true;
#if defined (CONFIG_MACH_GORDON)	
                    if (!strcmp(hw_ver, "c7zB"))  //==c7zB
                          boost_uv = BOOST_UV;
			else
			     boost_uv = BOOST_UV_SLOW;	
#else
                   boost_uv = BOOST_UV_SLOW;
#endif
			break;
