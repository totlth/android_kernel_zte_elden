...
static int32_t adm_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == RESET_EVENTS) {
		pr_err("adm_callback: Reset event is received: %d %d apr[%p]\n",
		...
		pr_err("chenjun:Resetting calibration blocks");
...
// chenjun
		...
		case ADM_CMD_MULTI_CHANNEL_COPP_OPEN: {
			struct adm_copp_open_respond *open = data->payload;
			pr_err("chenjun:%s: open copp_id %d\n",
