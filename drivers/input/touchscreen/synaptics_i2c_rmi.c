...
static int get_lcd_info(uint *xres, uint *yres)
{
	struct fb_info *info;
...
static int get_ts_pdt(struct synaptics_rmi4_data *ts)
{
	int ret=0;
	int nFd =1;
	int page_select=0;
	...
	do {
		...
		*/
		...
		if (ret < 0){
			pr_err("%s: failed to get ts i2c data %d\n",__func__,ret);
			return ret;
		}else{
			/*
			 * -----  RMI functions  -----
...
static void set_ts_platform_data(struct synaptics_rmi4_data *ts)
{
	struct proc_dir_entry *dir, *refresh;
	uint xres,yres=0;

	// init for engineer-query mode
	...
	if ( ts->f11.flag && (interrupt & ts->f11.intr_mask) ) 
	{
		...
		if (will_report) {
			for_each_set_bit(i, pointers_to_report, NR_FINGERS) {
				...
				#if defined (CONFIG_MACH_ARTHUR4G)
				//if (new_report_data[0].y >= 1900) input_report_key(ts->input_dev, BTN_TOUCH, !!new_report_data[0].z);
				#endif
				#if 0 //for test
				if( ts->has_vkeys )
				{
					if(new_report_data[0].y >= ts->lcd_active_area+10)
						input_report_key(ts->input_dev, KEY_MENU, !!new_report_data[0].z);
				}
				#endif
	...
	const struct i2c_device_id *id)
{
	...
	if (pdata){
		...
		ts->has_vkeys = pdata->has_vkeys;
		ts->lcd_active_area	= pdata->param;
	...
	ret = set_ts_work_state(ts, TS_POWER_ON); 
	if (ret < 0) {
...
static int synaptics_rmi4_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	set_ts_work_state(ts, TS_SUSPEND);
/*
	if (ts->power) {
		ret = ts->power(0);
		if (ret < 0)
...
*/
	...
	set_ts_work_state(ts, TS_RESUME);
