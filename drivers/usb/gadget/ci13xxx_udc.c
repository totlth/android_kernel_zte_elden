...
static int ep_dequeue(struct usb_ep *ep, struct usb_request *req)
{
	...
	{
		if(ep == NULL)
			printk(KERN_ERR "usb deugging:ep == NULL\n");
		if(req == NULL)
			printk(KERN_ERR "usb deugging:req == NULL\n");
		if(mReq->req.status != -EALREADY)
			printk(KERN_ERR "usb deugging:mReq->req.status != -EALREADY\n");
		if(mEp->desc == NULL)
			printk(KERN_ERR "usb deugging:mEp->desc == NULL\n");
		if(list_empty(&mReq->queue))
			printk(KERN_ERR "usb deugging:list_empty(&mReq->queue)\n");
		if(list_empty(&mEp->qh.queue))
			printk(KERN_ERR "usb deugging:list_empty(&mEp->qh.queue)\n");
