...
static int pm8xxx_pwm_led_blink(struct led_classdev *led_cdev,
				     ...
				     unsigned long *delay_off)
{
	...
	if(!strcmp(led->cdev.name, "led:battery_charging") || !strcmp(led->cdev.name, "led:battery_full") ){
		if(*delay_on != 0){
         		led->pwm_period_us = ((*delay_on) + (*delay_off)) * 1000 ;//us
			
         		led->cdev.brightness = led->cdev.brightness/20;
                        pwm_disable(led->pwm_dev);
				
		}else{
			led->pwm_period_us =  1000 ;//us			
		}
...
static int __devinit pm8xxx_led_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pcore_data->num_leds; i++) {
		...
		led_dat->cdev.blink_set               = pm8xxx_pwm_led_blink;
		#endif
