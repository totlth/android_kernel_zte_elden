...
Badass removes all of this "fast peaking" to the max frequency. On a typical system the cpu won't go above 918Mhz and therefore stay cool and will use less power.
To trigger a frequency increase, the system must run a bit @ 918Mhz with high load, then the frequency is bumped to 1188Mhz. If that is still not enough the governor
gives you full throttle. (this transition should not take longer than 1-2 seconds, depending on the load your system is experiencing)
Badass will also take the gpu load into consideration. If the gpu is moderately busy it will bypass the above check and clock the cpu with 1188Mhz. If the gpu is crushed under load,
badass will lift the restrictions to the cpu.
...
#if 1
/*power saving badass configuration*/
#define TWO_PHASE_FREQ                       918000
#define SEMI_BUSY_THRESHOLD			80
#define SEMI_BUSY_CLR_THRESHOLD		20
#define THREE_PHASE_FREQ                     1242000
#define BUSY_THRESHOLD				140
#define BUSY_CLR_THRESHOLD			110

#ifdef CONFIG_CPU_FREQ_GOV_BADASS_GPU_CONTROL
#define GPU_SEMI_BUSY_THRESHOLD			300
#define GPU_SEMI_BUSY_CLR_THRESHOLD	250
#define GPU_BUSY_THRESHOLD			       750
#define GPU_BUSY_CLR_THRESHOLD			600
#endif
#endif

#if 0
/*balanced configuration*/
#define TWO_PHASE_FREQ                     1026000
...
#if 0
/*performance oriented badass configuration*/
#define TWO_PHASE_FREQ                        1242000
#define SEMI_BUSY_THRESHOLD			8
#define SEMI_BUSY_CLR_THRESHOLD		4
#define THREE_PHASE_FREQ                     1512000
#define BUSY_THRESHOLD				120
#define BUSY_CLR_THRESHOLD			80

#ifdef CONFIG_CPU_FREQ_GOV_BADASS_GPU_CONTROL
#define GPU_SEMI_BUSY_THRESHOLD			150
#define GPU_SEMI_BUSY_CLR_THRESHOLD	50
#define GPU_BUSY_THRESHOLD			       450
#define GPU_BUSY_CLR_THRESHOLD			300
#endif
#endif
...
static void bds_freq_increase(struct cpufreq_policy *p, unsigned int freq)
{
	...
	if (bds_tuners_ins.powersave_bias)
      ...
      pr_debug("bds_freq_increase:cpu=%d,powersave_bias=%d,cur=%d,target_freq=%d,max=%d\n",cpu,bds_tuners_ins.powersave_bias,p->cur,freq,p->max);
...
static void bds_check_cpu(struct cpu_bds_info_s *this_bds_info)
{
	...
	for_each_cpu(j, policy->cpus) {
		...
		cur_load=load; //add by stone 20120829
...
/* Debug output for cpu control. Still needed for finetuning. */
         pr_debug("badass: cpu_phase=%d, counter=%d ,semi_busy_threshold=%d,busy_threshold=%d,cur=%d,max=%d\n'", phase, counter, bds_tuners_ins.semi_busy_threshold,bds_tuners_ins.busy_threshold,policy->cur,policy->max);
...
/*  Debug output for gpu control. Still needed for finetuning.*/
	pr_debug("badass: cpu_phase=%d, cpu_counter=%d,gpu_busy_phase=%d,gpu_busy_counter=%d ,gpu_busy_state=%d,cur=%d,max=%d\n'", phase, counter,gpu_busy_phase,gpu_busy_counter, gpu_busy_state,policy->cur,policy->max);
	...
	if (max_load_freq <
		...
		if (!bds_tuners_ins.powersave_bias) {
			pr_debug("bds_check_cpu_1:cur=%d,freq_next=%d\n",policy->cur,freq_next);
			...
			pr_debug("bds_check_cpu_2:cur=%d,freq_next=%d,freq=%d\n",policy->cur,freq_next,freq);
...
#ifdef CONFIG_CPU_FREQ_GOV_BADASS_2_PHASE
if (bds_tuners_ins.two_phase_freq <policy->max) 
	target_freq=bds_tuners_ins.two_phase_freq;
#endif

/*
#ifdef CONFIG_CPU_FREQ_GOV_BADASS_3_PHASE
if (bds_tuners_ins.three_phase_freq <policy->max) 
	target_freq=bds_tuners_ins.three_phase_freq;
#endif
*/

	pr_debug("bds_refresh_callback:cpu=%d,cur=%d,target_freq=%d,max=%d,min=%d\n",cpu,policy->cur,target_freq,policy->max,policy->min);

//modify by stone 20120830 reference zc in ondemand
		...
		unsigned int code, int value)
{
	int i;
       ...
       pr_debug(" bds_input_event:enable_bds_input_event=%d,dev_name=%s\n",enable_bds_input_event,handle->dev->name);
				   ...
				   unsigned int event)
{
	...
	pr_debug("cpufreq_governor_bds:cpu=%d,event=%d,ignore_nice=%d,powersave_bias=%d\n",cpu,event,bds_tuners_ins.ignore_nice,bds_tuners_ins.powersave_bias);
	return 0;
}
