...
int lcdinit_backlight_delay = 30;
...
void lcd_reset_esd(struct msm_fb_data_type *mfd)
{	
	msm_fb_blank_sub(FB_BLANK_POWERDOWN, fbi_list[0], true);
	msm_fb_blank_sub(FB_BLANK_UNBLANK, fbi_list[0], true);
}
EXPORT_SYMBOL(lcd_reset_esd);
...
//	printk("\nLCD %s:  blank_mode=%d  op_enable=%d\n", __func__, blank_mode,op_enable);
		...
		if (!mfd->panel_power_on) {
			...
			if (ret == 0) {
				...
				if(mfd->panel_info.type == MIPI_VIDEO_PANEL)
					msleep(lcdinit_backlight_delay);
				else
					msleep(30);
...
static int msm_fb_lcd_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	switch(LcdPanleID)
	{
		...
		case LCD_PANEL_4P3_NT35516_LG_TRULY:
			...
			break;
		case LCD_PANEL_4P3_OTM1280_OT_CMI:
			strcpy(module_name,"zteCMI_CMI(OTM1280)_720*1280_4.3Inch");
			break;
