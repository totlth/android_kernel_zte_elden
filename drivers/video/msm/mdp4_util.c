...
#define SAMPLES 15
static int64_t fb_timestamp[SAMPLES];
static int fb_idx=0;
//static short fps_flag=0;
int64_t last_fps_update;
static int fps_ok = 0;

void set_fps_ok(void)
{
	fps_ok = 0;
}
...
int is_fps_ok(void)
{
	...
	if (jiffies < 60*100)
		return 0;
	diff = cputime64_sub((cputime64_t)now,(cputime64_t)last_fps_update);
	if (diff > 300)
		fps_ok = 128;
...
#ifdef CONFIG_CPU_FREQ_GOV_SMARTASS2
	{
	int fb_pre_idx = (fb_idx == 0)?SAMPLES-1:fb_idx-1;
	cputime64_t diff;

	last_fps_update = ktime_to_ms(ktime_get());
	fb_timestamp[fb_idx] = last_fps_update;
	diff = cputime64_sub((cputime64_t)fb_timestamp[fb_idx],(cputime64_t)fb_timestamp[fb_pre_idx]);
