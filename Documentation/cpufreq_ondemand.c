...
//extern unsigned int is_screen_refesh_above_33fps(void);
extern u32 fb_fps;
u32 save_fps = 0;
u64 save_timestamp = 0;
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	do_div(now, 1000 * 1000);
	fps = (fb_fps - save_fps)*1000/(u32)(now-save_timestamp);
	save_fps = fb_fps;
	save_timestamp = (u64) now;
	...
	if (max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) {
		...
		if (counter < 5) {
			...
			if (debug_mask)
			     printk("tcd counter no inc, %u", fps);
			...
			//tcd add
//			if ((phase == 1) && (is_screen_refesh_above_33fps() == 1)){
//				phase = 0;
//				counter = 0;
//				printk("fps , bingo ");
//			}
